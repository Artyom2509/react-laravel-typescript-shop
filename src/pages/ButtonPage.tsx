...
const ButtonPage: React.FC = () => {
	const [cSelected, setCSelected] = useState<Array<string | number>>([]);
	const [rSelected, setRSelected] = useState<null | number>(null);

	const onCheckboxBtnClick = (selected: string | number) => {
		const cSelect = cSelected.concat();
		const index = cSelect.indexOf(selected);

		if (index < 0) cSelect.push(selected);
		else cSelect.splice(index, 1);
